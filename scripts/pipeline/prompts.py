"""Prompt templates for the GIKAI AI pipeline.

All prompts are centralized here for transparency and auditability.
These prompts are open-sourced as part of GIKAI's political neutrality guarantee.

Each prompt is split into two parts:
    *_INSTRUCTIONS — the static rules / examples / output format. This block
                      is identical across every call and is marked with
                      ``cache_control: {"type": "ephemeral"}`` by the caller
                      so subsequent calls within the 5-minute window only pay
                      for the variable input. The literal rule text is
                      unchanged from the original combined prompt; only the
                      block ordering changed.
    *_INPUT_TEMPLATE — the per-call variable portion (meeting metadata and
                       speeches) that is filled in with ``.format(...)``.

The system prompt is also unchanged.
"""

# ---------------------------------------------------------------------------
# Phase B: Topic grouping — one API call per meeting
# ---------------------------------------------------------------------------

GROUPING_SYSTEM = """\
あなたは国会の議事録を構造化するアシスタントです。
政治的に中立な立場で、与党・野党を区別せず同じ基準で処理してください。"""

GROUPING_INSTRUCTIONS = """\
国会の発言一覧をテーマ（議題・話題）単位でグルーピングしてください。

## ルール
- 委員長の手続き発言（開会宣言、議案朗読、採決宣告など）はどのテーマにも含めない
- speechOrderの連続性を重視する（同テーマの発言は通常連続する）
- テーマ名は具体的に（「AI規制法案の高リスク定義」など、何の議論かわかるように）
- topicTagは5文字以内の短縮タグ
- topicColorはテーマの分野に応じて選択:
  経済・産業=#3b82f6, 環境・エネルギー=#22c55e, 安全保障=#ef4444,
  社会保障=#8b5cf6, 教育=#f59e0b, 外交=#06b6d4, 法務=#6366f1,
  行政改革=#f97316, 災害=#dc2626, その他=#6b7280
- summaryはスレッド全体の要点を60字以内で。報告文体（「〜について議論した」）は使わず、
  議論の核心を直接表現する
- contextDescriptionは、このテーマの背景を知らない読者向けの簡潔な説明（80字以内）。
  法案名が含まれる場合はその法案が何を目的としたものか、政策テーマの場合は何が問題になっているか。
  例: 「AIの開発・利用に関するリスク分類と規制の枠組みを定める法案。2024年に閣議決定。」
- legislationName: 議論の対象となっている法案・法律名があれば正式名称（なければnull）

## 出力（JSONのみ、他のテキストは不要）
```json
{
  "threads": [
    {
      "topic": "テーマ名",
      "topicTag": "短縮タグ",
      "topicColor": "#hex",
      "summary": "スレッド全体の一行サマリー（60字以内）",
      "contextDescription": "このテーマの背景説明（80字以内）",
      "legislationName": "正式な法案・法律名（あれば）",
      "speechOrders": [4, 5, 6, 7]
    }
  ]
}
```"""

GROUPING_INPUT_TEMPLATE = """\
以下は国会の{house}{meeting}（{date}）の発言一覧です。
各発言の冒頭部分と発言者情報を示します。

## 発言一覧
{speeches}"""


# ---------------------------------------------------------------------------
# Phase C: Per-thread speech summarization — one API call per thread
# ---------------------------------------------------------------------------

SUMMARY_SYSTEM = """\
あなたは国会の議事録を要約するアシスタントです。
政治的に中立な立場で、与党・野党を区別せず同じ基準で処理してください。"""

SUMMARY_INSTRUCTIONS = """\
発言について、要約・分類・キーワード抽出を行ってください。

## 要約の文体ルール（最重要）
要約は発言内容そのものを簡潔に表現する。報告文体は絶対に使わない。

❌ NG（報告文体）:
「AI定義が曖昧すぎると指摘し、EUとの比較を求めた。」
「影響試算を提出すると約束した。」

✅ OK（発言内容そのもの）:
「AI定義が曖昧すぎて、レコメンドエンジンまで規制対象になりうる。EUとの比較と試算を出せ。」
「採決前に影響試算を必ず提出する。国会報告義務の明記も検討する。」

## レベル別指示
- easy（やさしく）: 難しい言葉を言い換え、断定調（〜だ、〜する）、60〜80字
- teen（標準）: 専門用語には括弧で短い補足、体言止めOK、80〜100字
- adult（詳しく）: 政治・法律用語そのまま、論点を端的に、体言止めOK、60〜80字

## tension分類
- 追及: 野党が政府・与党を問いただす質問
- 答弁: 大臣・政府委員の回答
- 再追及: 答弁が不十分として重ねて追及
- 確認: 事実関係の確認、穏やかな質疑
- 割込み: 議事進行発言、野次、委員長の制止など

## キーワード抽出ルール
発言の**議論対象**を表す一般的な政策名詞を3つ抽出する。トレンド集計で同一話題が1つにまとまるよう、**最も一般的な短い形**を選ぶこと。

❌ NG:
- 人名・政治家名（例: 高市早苗、石破総理） — プロフィールページで扱うので不要
- 省庁・機関名（例: 厚生労働省、内閣府、国立国会図書館） — 議論の舞台であって話題ではない
- 手続き語（例: 委員長一任、委員長報告、趣旨説明、附帯決議、就任挨拶、参考人、有識者会議） — 議事運営の語
- 発言断片・レトリック（例: 国権の最高機関、主権者の代表、責任ある積極財政） — 修辞で政策対象ではない
- 派生形・修飾形（例: 令和8年度予算成立、ガソリン暫定税率廃止、能登地震） → 根幹の形で（例: 令和8年度予算、ガソリン暫定税率、能登半島地震）

✅ OK:
- 政策・法案名（例: 令和8年度予算、AI規制、二地域居住、地域医療構想）
- 具体的な事件・対象（例: 能登半島地震、ホルムズ海峡、東日本大震災）
- 政治用語（例: 抑止力、人材確保、消費税減税、賃上げ促進税制）

同じ概念を指す表記が複数あり得る場合は、常に最も一般的で短い形を選ぶ。

## コミットメント抽出
答弁の中で大臣・政府委員が具体的に約束・表明したことを抽出する。
例: 「採決前に影響試算を提出する」「次の国会で法案を出す」「検討会を設置する」
曖昧な「検討する」「努力する」は含めない。具体的なアクションのみ。

## 出力（JSONのみ、他のテキストは不要）
```json
{
  "speeches": [
    {
      "speechOrder": 4,
      "tension": "追及",
      "keywords": ["キーワード1", "キーワード2", "キーワード3"],
      "quote": "原文から最も重要な一文をそのまま抜粋（改変不可）",
      "summaries": {
        "easy": "...(60-80字)",
        "teen": "...(80-100字)",
        "adult": "...(60-80字)"
      }
    }
  ],
  "commitments": ["具体的な約束1", "具体的な約束2"]
}
```"""

SUMMARY_INPUT_TEMPLATE = """\
以下は{house}{meeting}の「{topic}」に関する発言です。

## 発言一覧
{speeches}"""


# ---------------------------------------------------------------------------
# Phase D: Meeting-level outcome extraction (votes, resolutions)
# ---------------------------------------------------------------------------
# This prompt is short enough that caching is not worthwhile (cached prefix
# must reach 1,024 tokens). Kept as a single combined template.

OUTCOME_SYSTEM = """\
あなたは国会の議事録から採決結果と附帯決議を抽出するアシスタントです。"""

OUTCOME_PROMPT = """\
以下は国会の{house}{meeting}（{date}）の委員長発言（手続き部分）です。
この中から採決結果と附帯決議を抽出してください。

## 抽出ルール
- 採決があれば結果（可決/否決）を記録
- 附帯決議があればその要旨を100字以内で要約
- 採決がない場合（所信聴取のみ、質疑のみ）は result を null にする
- 質疑のみで採決に至っていない場合は status を "ongoing" にする

## 委員長発言
{procedural_speeches}

## 出力（JSONのみ）
```json
{{
  "result": "可決" または "否決" または null,
  "resolution": "附帯決議の要旨（あれば）" または null,
  "status": "resolved" または "ongoing"
}}
```"""
