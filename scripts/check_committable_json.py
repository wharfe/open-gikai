...
#   reads members first and exits 1 by design (`MembersUnreadable`), and if
#   that is skipped, `validate-data.mjs`'s `checkMembers` (a bare
#   `JSON.parse`, scripts/validate-data.mjs:99) throws on it too, with no
#   `continue-on-error`. Either way the job is dead before this step exists.
#   The wrong-*shape* case ([] instead of an object) is different:
#   `checkMembers` does NOT die on it — `JSON.parse("[]")` succeeds, and the
#   member-lookup loop below it (scripts/validate-data.mjs:100-118) just
#   misreports rather than throwing. It is `enrich-members.mjs`, the very next
#   step, that dies on that case: `main()` exits 1 on a non-object top level
#   (verified: `echo '[]' > x.json && node scripts/enrich-members.mjs
#   --members-path x.json` → "is not a JSON object", exit 1) — *after*
#   validate-data.mjs has already tolerated the same file, and *before*
#   generate-feeds.js / generate-sitemap.mjs get a turn to bare-parse it too.
#   So today both ways `data/members.json` can be unreadable are dead before
#   this checker runs — a syntax error at `checkMembers`, a bad top-level
#   shape at `enrich-members.mjs` — which makes the
#   `("data/members.json", dict)` entry below completely dormant: it is not
#   catching a run-introduced corruption (as argued above) *and* it is not
#   catching HEAD-side corruption either, because HEAD's own copy hits the
#   same two upstream deaths before a corrupt file could ever reach this
#   step. It is kept for the shape of this list, not because it is presently
#   protecting anything.
...
# this file. Most of them still see `HEAD`-side corruption even though a
# run-introduced one is caught upstream; `data/members.json` is the exception,
# detailed above — dormant against both, not just the run-introduced case. Do
# not read any of their presence as a claim of protection.
CHECKS = (
