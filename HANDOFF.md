...
> ⚠️ **このドキュメントは歴史的な初期スペック（2026-03時点）です。**
> 現行の正典は CLAUDE.md と実コードです（例: Next.js は 16、公開ドメインは open-gikai.net、Phase 1 は本番稼働済み）。
> ただし「データ構造（フロントエンド用JSON）」節の型スキーマ（`Thread` / `Speech` / `Member`）のみ CLAUDE.md から現役参照されています。
> それ以外の記述は更新されません。
